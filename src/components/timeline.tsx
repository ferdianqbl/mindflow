    ...
    return (
      <div className="flex h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-white/5 bg-slate-950/10 p-6 text-center">
        ...
        <p className="mt-1 max-w-xs text-xs leading-5 text-slate-400">
          Start your first focus session and write an accomplishment log to
          populate your timeline.
        </p>
      </div>
    );
  }
        ...
        {logs.map((log) => {
          ...
          return (
            <div key={log.id} className="relative flex items-start pl-14 group">
              ...
              <div
                className={`absolute left-2.5 flex h-7.5 w-7.5 items-center justify-center rounded-full border bg-[#070a13] shadow-lg transition-transform duration-200 group-hover:scale-105 ${config.color}`}
              >
