...
import { BookOpen, Bug, Code, Palette, Settings, Users, X } from "lucide-react";
...
export default function JournalModal({
  ...
  const categories = [
    {
      id: "coding" as WorkCategory,
      name: "Coding",
      icon: Code,
      style:
        "border-cyan-500/20 text-cyan-400 bg-cyan-500/5 hover:bg-cyan-500/10 focus:ring-cyan-500/30",
    },
    {
      id: "debugging" as WorkCategory,
      name: "Debugging",
      icon: Bug,
      style:
        "border-red-500/20 text-red-400 bg-red-500/5 hover:bg-red-500/10 focus:ring-red-500/30",
    },
    {
      id: "design" as WorkCategory,
      name: "UI/UX Design",
      icon: Palette,
      style:
        "border-pink-500/20 text-pink-400 bg-pink-500/5 hover:bg-pink-500/10 focus:ring-pink-500/30",
    },
    {
      id: "learning" as WorkCategory,
      name: "Learning",
      icon: BookOpen,
      style:
        "border-amber-500/20 text-amber-400 bg-amber-500/5 hover:bg-amber-500/10 focus:ring-amber-500/30",
    },
    {
      id: "meeting" as WorkCategory,
      name: "Meeting",
      icon: Users,
      style:
        "border-indigo-500/20 text-indigo-400 bg-indigo-500/5 hover:bg-indigo-500/10 focus:ring-indigo-500/30",
    },
    {
      id: "operations" as WorkCategory,
      name: "Operations",
      icon: Settings,
      style:
        "border-slate-500/20 text-slate-400 bg-slate-500/5 hover:bg-slate-500/10 focus:ring-slate-500/30",
    },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        e instanceof Error ? e.message : "Failed to log accomplishment.";
      setErrorMsg(message);
    } finally {
      setLoading(false);
    }
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-2 rounded-xl border px-3 py-2.5 text-xs font-medium outline-none transition-all duration-200 focus:ring-1 active:scale-95 ${
                      isSelected
                        ? cat.style
                            .replace("bg-", "bg-selected-")
                            .replace("border-", "border-selected-") +
                          " border-white/20 bg-white/10 ring-1 ring-white/20 text-slate-100"
                        : cat.style
                    }`}
                  >
